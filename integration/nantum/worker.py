...
class FetchWorker(BaseFetchWorker):
    ...
    def fetch(self) -> None:
        ...
        start_date = truncate(
            min(missed_hours).subtract(days=1), 
            level="hour"
        ) 
        
        end_date = truncate(
            max(missed_hours).add(days=1),
            level="hour"
        )            
        ...
        try:
            result = self._request_data(
                ...
                params={
                    ...
                    "query[date][gte]=": format_date(
                        start_date,
                        self.__date_format__
                    ),
                    "query[date][lt]=": format_date(
                        end_date,
                        self.__date_format__
                    ),
                },
        ...
        file_info = RawFetchFile(
            ...
            body=dumps(
                self._factory.dump(raw_data), sort_keys=True, indent=4
            ),
        )
    ...
    def unbundle_fetch_data_consumer(
        ...
        while True:
            ...
            for hr in missed_hours:
                mtr_cfgs = self._missed_hours_queue.get(hr)
                if mtr_cfgs:
                    fl = DocFetchFile(
                        doc=doc,
                        meters=mtr_cfgs,
                    )
                    fl.timestamps.put(hr)
                    self._fetched_files_queue.put(fl)
                else:
                    self._th_logger.warning(
                        f"Cannot find related meter for hour '{hr}'. Skip"
                    )       
...
class StandrdizeWorker(BaseStandardizeWorker):
    ...
    def _standardize(self, raw_file_obj: DataFile) -> List[DataFile]:
        ...
        raw_data = self._get_data_df(
            self._factory.dump(raw_file_obj.doc.readings)
        )

        while not raw_file_obj.timestamps.empty():
            mtr_hr = raw_file_obj.timestamps.get()
            meter_hour = format_date(
                truncate(parse(mtr_hr), level="hour"),
                CFG.PROCESSING_DATE_FORMAT
            )
            while not raw_file_obj.meters.empty():
                ...
                meter = stndrdz_func(
                    data=raw_data.loc[raw_data["hour"] == meter_hour].to_dict("records"),
                    mtr_cfg=mtr_cfg,
                )
                if meter:
