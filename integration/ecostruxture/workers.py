...
from integration.willow.data_structures import DataFile
...
class FetchWorker(BaseFetchWorker):
    ...
    def _get_mail_processed_folder(self, mailbox: MailBox) -> str:
        processed_folder = self._joinmailpath(
            self.__mail_root_folder__, 
            self.__mail_processed__
        )
        ...
        with self._get_mail_connection() as mailbox:
            ...
            for msg in msgs:
                ...
                for attachement in attachements:
                    self._fetched_atachments_q.put(
                        (
                            msg.uid,
                            attachement
                        )
    ...
    def _get_meter_hour_form_excel(self, workbook) -> Set[DateTime]:
        ...
        date = sheet.cell(
            row =self.__sheet_first_value_row__,
            column=self.__sheet_date_column__
        ).value

        dt = truncate(parse(date), level="hour") 
    ...
    def unbundle_fetch_data_consumer(
        ...
        while True:
            ...
            wb = self._read_excel(BytesIO(attachement.payload))
            day_hours = self._get_meter_hour_form_excel(wb)

            hours = set(self._missed_hours_queue.keys()).intersection(
                day_hours
            )
            ...
            if hours:
                ...
                for hr in hours:
                    with self._lock:
                        mtr_cfgs = self._missed_hours_queue.get(hr, None)
...
class StandrdizeWorker(BaseStandardizeWorker):
    ...
    def __calc_consumption(row):
        row["Consumption"] = math.fabs(
            row.endConsumption - row.startConsumption
        )
        return row
